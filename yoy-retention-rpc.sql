-- ========================================
-- YOY & RETENTION RPC FUNCTIONS
-- ========================================
-- Functions for complex metrics that require cross-season analysis

-- ========================================
-- 1. YEAR-OVER-YEAR GROWTH FUNCTIONS
-- ========================================
...
BEGIN
    ...
    SELECT 
        ...
        cs.boxes as current_boxes,
        COALESCE(ps.boxes, 0) as previous_boxes,
        (cs.boxes - COALESCE(ps.boxes, 0)) as yoy_boxes_growth,
        CASE 
...
RETURNS TABLE(
    ...
    yoy_boxes_growth INTEGER,
    yoy_boxes_growth_pct NUMERIC,
    current_importers INTEGER,
    previous_importers INTEGER,
    yoy_importers_growth INTEGER,
...
BEGIN
    ...
    SELECT 
        ...
        END as yoy_kg_growth_pct,
        cs.boxes as current_boxes,
        ps.boxes as previous_boxes,
        (cs.boxes - ps.boxes) as yoy_boxes_growth,
        CASE 
            WHEN ps.boxes > 0 THEN ((cs.boxes - ps.boxes) / ps.boxes) * 100
            ELSE NULL
        END as yoy_boxes_growth_pct,
        cs.importers as current_importers,
        ps.importers as previous_importers,
        (cs.importers - ps.importers) as yoy_importers_growth,
        CASE 
...
-- ========================================
-- 2. IMPORTER RETENTION FUNCTIONS
-- ========================================
...
RETURNS TABLE(
    ...
    exporter_name TEXT,
    previous_importers INTEGER,
    retained_importers INTEGER,
    new_importers INTEGER,
    churned_importers INTEGER,
...
BEGIN
    ...
    SELECT 
        ra.exporter_id,
        ra.exporter_name,
        ra.previous_importers,
        ra.retained_importers,
        ra.new_importers,
        ra.churned_importers,
        CASE 
...
-- YoY and Retention RPC functions have been created.
-- These functions provide complex cross-season analysis for:
-- 1. Year-over-Year growth (kilograms, boxes, importers)
-- 2. Importer retention rates (by exporter and global)
-- 3. Acquisition and churn metrics
